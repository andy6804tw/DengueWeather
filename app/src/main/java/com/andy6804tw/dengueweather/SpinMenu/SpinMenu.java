...
public class SpinMenu extends FrameLayout {
     ...
     * 菜单打开关闭动画帮助类
     */
    private static SpinMenuAnimator spinMenuAnimator;
     ...
     * 当前菜单状态，默认为已关闭
     */
    private static int menuState = MENU_STATE_CLOSED;
        ...
        if (init && smItemLayoutList.size() > 0) {
            ...
            for (int i = 0; i < smItemLayoutList.size(); i++) {
                ...
                if (i == 0) { // 初始菜单的时候，默认显示第一个 Fragment
                    ...
                    LayoutParams pagerFrameParams = new LayoutParams(MATCH_PARENT, MATCH_PARENT);
